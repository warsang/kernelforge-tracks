...
| sogen core — JS glue vendored, wasm boot-fetched | momo5502/sogen @ sogen.dev deploy 2026-08-25 (packages/sogen-runtime/vendor + apps/web/public/sogen) | GPL-2.0 | same policy as unicorn; sha256 pins in vendor/README.md; `npm run vendor:sogen` fetch tool; JS reference backend stays as test fallback |
| Monaco editor | microsoft/monaco-editor (npm dep) | MIT | attribution in package.json |
| pyre decompiler build pipeline (recipe only) | ant4g0nist/pyre | MIT | recipe adopted in packages/ghidra-decompiler/vendor/README.md; no code copied |
...
| Ghidra decompiler engine | NationalSecurityAgency/ghidra (Features/Decompiler cpp), wasm wrapper | Apache-2.0 | keep NOTICE alongside vendor dir; record tag/emcc version/sha256; build via the adopted pyre pipeline |
